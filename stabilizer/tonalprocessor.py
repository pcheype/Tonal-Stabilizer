...
class TonalProcessor(QThread):
    ...
    def resize_keep_aspect(bgr, w):
        """Resize simple en gardant le ratio"""
        h, W = bgr.shape[:2]
    ...
    def estimate_affine_dominant_motion(prev_gray, curr_gray):
        """Tracking des points pour estimer le mouvement entre deux frames"""
    ...
    def build_overlap_correspondences(self, u_k, u_t, A_tk):
        """Methode pour trouver les correspondances"""
        Hk, Wk = u_k.shape[:2]
        Ht, Wt = u_t.shape[:2]
        ...
        #Masque pour garder les pixels cohérents
    ...
    def estimate_alpha_gamma_logLS(uk_vals, ut_vals, eps=1e-6):
        """Moindres carrées pour trouver les solutions analytiques (voir papier)"""
    ...
    def apply_parametric_correction_u8(frame_bgr_u8, alpha, gamma, lam):
        """Application de la correction en unit8"""
    ...
    def dominant_motion_vector(A_tk, W, H):
        """Calcul du vecteur de mouvement principal"""
    ...
    def stabilize_to_memory(self):
        ...
        self.processed_frames.append(out_prev)
        ...
        while True:
            ...
            #Estimation du mouvement/couleur
            ...
            #Si on a assez de points communs, on corrige.
            if len(idx_x) > 0 and np.count_nonzero(good) >= int(math.ceil(self.OMEGA_FRAC * len(idx_x))):
                ...
                #Calcul de l'attenuation selon le mouvement 
